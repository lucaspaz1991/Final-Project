...
export class ModalDepositoComponent implements OnInit{
  ...
  // Agrega o modifica deposito en base al autocompletado del formulario
  GuardarEditardeposito(){
    ...
    this.arrayDepositos.forEach(element => {
      ...
      if(element.codigo === this.formularioDeposito.value.codigo)
        this.aux++;
    });
        ...
        this.utilidadService.mostrarAlerta("Codigo del depósito repetido","Error");
          ...
          if(deposito.capacidadMaxima < 0){
            ...
            this.depositoService.AddDeposito(deposito).subscribe({
              next: (data) => {
                  this.utilidadService.mostrarAlerta("El deposito fue registrado","Exito");
                  this.modalActual.close("true")
              },
              error: (e) => {
                this.utilidadService.mostrarAlerta("No se pudo registrar el deposito","Error")
  ...
  // Obtener la info del deposito que se selecciona para editar
